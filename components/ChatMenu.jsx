            ...
            const updatedMessages=chatDoc.data().messages.map(
                (message)=>{
                    message.deleteChatInfo={
                        ...message.deleteChatInfo,//new messsage as well as old message gets true
  ...
  return (
    ...
    <div className="w-[200px] absolute top-[70px] right-5 bg-c0 z-10 rounded-md overflow-hidden" >
      <ul className="flex flex-col py-2" >
       {!IamBlocked &&( <li className="flex items-center py-3 px-5 hover:bg-black cursor-pointer"         onClick={(e)=>{
                e.stopPropagation()
                handleBlock(isUserBlocked?"unblock":"block")
            }} >
             {isUserBlocked?"Unblock":"Block"}
        </li>)}
        <li className="flex items-center py-3 px-5 hover:bg-black cursor-pointer" onClick={(e) => {e.stopPropagation() 
        handleDelete() 
        setShowMenu(false) }  } >
            Delete Chat
        </li>
