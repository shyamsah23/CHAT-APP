...
const Chats = () => {
   const {
      ...
      chats,setChats,dispatch,data,resetFooterStates
   }=useChatContext();
   ...
   const isUsersFetchRef=useRef(false);
      ...
      const getChats =() => {
         const unsub =onSnapshot(doc(db,"userChats",currentUser?.uid),(doc) => {
            if(doc.exists())
            {
               ...
               console.log(data)
               setChats(data);
               if(!isBlockExecutedRef?.current && isUsersFetchRef?.current && users)
               {
   ...
   .filter(
      ([, chat]) => chat?.userInfo?.displayName?.toLowerCase().includes(search?.toLocaleLowerCase()) || 
  ...
  return (
   <div className="flex flex-col h-full" >
      <div className="shrink-0 sticky -top-[20px] x-10 flex justify-center w-full bg-c2 py-5" >
      ...
      <ul className="flex flex-col w-full my-5 gap-[2px]" >
         ...
         >
            ...
            <div className="flex flex-col gap-1 grow relative" >
               ...
               <p className="text-sm line-clamp-1 braeak-all text-c3" >
                  {chat[1].lastMessage?.text || (chat[1].lastMessage?.img && "image" ) || "Send First Message"}
                   </p>

               {!!unreadMsgs?.[chat[0]]?.length && ( <span className="absolute right-0 top-7 min-w-[20px] 
                   h-5 rounded-full bg-red-500 flex
